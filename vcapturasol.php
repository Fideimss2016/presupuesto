<?php
	...
	if($result!=0)
	{
		...
		else if($status==1 && $numerosc!=0)
		{
										...
										$sqlUpdate = mysql_query("UPDATE obras SET status=0 where clave='$clave'", $connect) or die(mysql_error());
										if($sqlUpdate)
										{
										echo "El Plan Rector 2017 correspondiente al &Aacute;rea de Obras se ha liberado!!!";
										$sqlUpdate = mysql_query("UPDATE vobo SET jefe_o=0 where clave='$clave'", $connect) or die(mysql_error());
										
										}
										else
										{
										echo "El Plan Rector 2017 correspondiente al &Aacute;rea de Obras no ha sido liberado!!!";										
										}
