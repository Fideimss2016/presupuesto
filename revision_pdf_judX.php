...
*/
	...
	$pdf->SetXY(86,$y);
	$pdf->MultiCell(256,4,"EGRESOS",1,'C',1);
	...
	$pdf->SetXY(266,$y1);
	$pdf->MultiCell(56,4,"CAPITULO 6",1,'C',1);

	$pdf->SetXY(266,$y2);
	$pdf->MultiCell(20,4,"0601",1,'C',1);
	...
	while($row = mysql_fetch_array($result))
	{
		...
		$result61 = mysql_query("select sum(monto) as total_gasto_61 from obras where clave = '$clave' and clave_par = '0601' and activo = 1", $connect);
		...
		$result62 = mysql_query("select sum(monto) as total_gasto_62 from obras where clave = '$clave' and clave_par = '0602' and activo = 1", $connect);
		...
		$result63 = mysql_query("select sum(monto) as total_gasto_63 from obras where clave = '$clave' and clave_par = '0603' and activo = 1", $connect);
		$totalregistros = mysql_num_rows($result61);
		while($row = mysql_fetch_array($result61))
		{
...
//		$diferencia=$ingreso_total-$total;
	...
	$pdf->SetXY(266,$y);
	$pdf->MultiCell(20,4,number_format($total_gasto_61,2),1,'R',0);
	...
	$pdf->SetXY(266, $y);
	$pdf->MultiCell(20, 4,number_format($gtotal_gasto_61, 2),1,'R',0);
