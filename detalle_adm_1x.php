...
//	include "valida_seguridad.php";
	...
	{ 
		...
		$sqlUpdate = mysql_query("UPDATE vobo SET notifica = 1 WHERE clave='$clavenot'", $connect) or die(mysql_error());
		if($sqlUpdate)
		{							
			...
			$headers .= "Cc: leticia.giles@fideimss.org.mx, macrina.bravo@fideimss.org.mx, jantonio.olivarez@fideimss.org.mx, $email_1, $email_2, $email_3\r\n";
	...
	if (isset($_REQUEST['p']))
	{
		...
		if ($p == 't') 
		{
			...
			$sqlUpdateps = mysql_query("UPDATE vobo SET jefe_p = 1 WHERE clave='$clapers'", $connect) or die(mysql_error());
		}	
	}
	...
	if (isset($_REQUEST['e']))
	{
		...
		if ($e == 't') 
		{
			...
			$sqlUpdateps = mysql_query("UPDATE vobo SET jefe_e = 1 WHERE clave='$clapegs'", $connect) or die(mysql_error());
		}	
	}
	...
	if (isset($_REQUEST['in']))
	{
		...
		if ($in == 't') 
		{
			...
			$sqlUpdatein = mysql_query("UPDATE vobo SET jefe_i = 1 WHERE clave='$clapin'", $connect) or die(mysql_error());
		}	
	}
	if (isset($_REQUEST['o']))
	{
		$o = $_REQUEST['o'];
		$clapo = $_REQUEST['clave'];

		if ($o == 't') 
		{
			echo "si cambia IN $o $clapo";		
			$sqlUpdateo = mysql_query("UPDATE vobo SET jefe_o = 1 WHERE clave='$clapo'", $connect) or die(mysql_error());
		}	
	}
	...
	while($row = mysql_fetch_array($result))
	{
		...
		echo "<tr>
		      ...
		      <td bgcolor=\"$celda\" align=\"center\"><span class=\"white\">Presupuesto 2016 (ingresos)</span></td>
		...
		while($row = mysql_fetch_array($result1))
		{
			...
			echo "
		      ...
		      <td bgcolor=\"$c2\" align=\"left\" rowspan=\"2\"><span class=\"$color\">$clave $desc_uops </span><a href=\"vcapturas.php?clave=$clave\" class=\"grey\" target=\"_blank\">ver detalle</a>
			   ...
			   | <a href=\"obras_excelx.php?clave=$clave\" class=\"grey\" target=\"_blank\">Obra a excel</a>
			...
			if($tipo_usuario == 'ADM' || $tipo_usuario == 'SUP')
			{
				if($statuse == 3 && $statuso == 3 && $statusp == 3)
				{
					if($notifica == 0)
						...
						echo "<form name='form1' action='detalle_adm_1x.php' method='POST'>";
		  	...
		  	if($statusi == 0)
			{
				...
				{
					echo "    <td bgcolor=\"$c2\" align=\"center\"><span class=$color><a href=\"detalle_adm_1x.php?in=t&&clave=$clave\" title=\"capturando registros\"><img src=\"capturando.png\" width=\"20\" height=\"20\" /></a></span></td>";
				}
			}
			else if($statusi == 1)
			{
			...
			{
				echo "    <td bgcolor=\"$c2\" align=\"center\"><span class=$color><a href=\"\"  title=\"programa aprobado\" target=\"_blank\"><img src=\"aprobado.png\" width=\"20\" height=\"20\" /></a></span></td>";
			}
	  		if($statuse == 0)
			{
				...
				{
					echo "    <td bgcolor=\"$c2\" align=\"center\"><span class=$color><a href=\"detalle_adm_1x.php?e=t&&clave=$clave\" title=\"capturando registros\"><img src=\"capturando.png\" width=\"20\" height=\"20\" /></a></span></td>";
				}
			}
			else if($statuse == 1)
			{
	  		...
	  		if($statuso == 0)
			{
				...
				{
					echo "    <td bgcolor=\"$c2\" align=\"center\"><span class=$color><a href=\"detalle_adm_1x.php?o=t&&clave=$clave\" title=\"capturando registros\"><img src=\"capturando.png\" width=\"20\" height=\"20\" /></a></span></td>";
				}
			}
			else if($statuso == 1)
			{
	  		...
	  		if($statusp == 0 || $statusp == 10)
			{
				...
				{
					echo "    <td bgcolor=\"$c2\" align=\"center\"><span class=$color><a href=\"detalle_adm_1x.php?p=t&&clave=$clave\" title=\"capturando registros\"><img src=\"capturando.png\" width=\"20\" height=\"20\" /></a></span></td>";
				}
			}
			else if($statusp == 1)
			{
