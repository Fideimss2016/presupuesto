<?php
	...
	$result = mysql_query("SELECT id_conse_metas,id_emp,clave,clave_del,clave_uops,cta_der,cta_noder, enero,dh1,ndh1,horas1,febrero,dh2,ndh2,horas2,marzo,dh3,ndh3,horas3,abril,dh4,ndh4,horas4,mayo,dh5,
	ndh5,horas5, junio,dh6,ndh6,horas6,julio,dh7,ndh7,horas7,agosto,dh8,ndh8,horas8,septiembre,dh9,ndh9,horas9,octubre,dh10,ndh10,horas10, noviembre,dh11,ndh11,horas11,diciembre,dh12,ndh12,horas12,
	total_gastop,fecha_cap,id_usuario,vobo,status,estrategia FROM metas WHERE id_emp=$id_emp and clave='$clave'", $connect);
	$totalregistros = mysql_num_rows($result);
	$valcolor == 0;
	while($row = mysql_fetch_array($result))
	{
	...
	$pdf->SetTextColor(0,0,0);
	$pdf->SetDrawColor(51,51,51);
	$resultc = mysql_query("select id_conse_programa,mes,semanas,principiantes,intermedios,avanzados,observaciones from programa where clave='$clave' and id_emp=$id_emp order by mes, id_conse_programa", 
	$connect);
	$totalregistros = mysql_num_rows($resultc);
	while($row=mysql_fetch_array($resultc))
	{
