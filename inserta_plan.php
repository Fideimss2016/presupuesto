...
echo "		</tr>";
								...
								$result=mysql_query("select max(conse_plan) as conse_plan from plan where clave='$clave' and id_emp=$id_emp", $connect);
								$totalregistrosm1=mysql_num_rows($result);
								while($row=mysql_fetch_array($result))
								{
								$conse_plan=$row['conse_plan'];
								}
								$conse_plan=$conse_plan+1;
								...
								$resultadom = mysql_query("INSERT INTO plan(conse_plan,id_emp,clave,clave_del,clave_uops,tema,objpar,tecnica,material,docente,usuario,sesiones,horasxsesion,fecha_cap,id_usuario,vobo,status)
								 						 VALUES($conse_plan,$id_emp,'$clave','$clave_del','$clave_uops','$tema','$objpar','$tecnica','$material','$docente','$usuario',$sesion,$horases,'$feccaptura','$usu',0,0)", $connect);
...
echo "<br \>";
								...
								if($resultadom)
								{echo "<span class=\"spblue\">El plan se ha registrado satisfactoriamente</span>";
								 echo "<br>";
								 echo "<a href=\"plan.php?id_emp=$id_emp&&clave=$clave\" class=\"spwhite\">Capturar otro</a>";
								}//if($resultado)
								else
								{
								 echo "<span class=\"spred\">Error en el registro del plan!!!</span>";
								 echo "<br>";
